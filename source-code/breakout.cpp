...
enum {White=0, Black, Red, Orange, Yellow,
      Green, DarkGreen, Blue, NavyBlue, DarkViolet}; // Available colours
   ...
   void checkCollision(vector<Brick *> &bVector, Paddle &paddle,
      ...
      if (score.getNumBricks() == 0) {
        quit = true;
      	...
      	quit = true;
      } else if (hitBrick(bVector, score, xinfo, ballBrickCollisionType)) {
...
void initX(int argc, char *argv[], XInfo &xinfo) {
   ...
   char color_vals[256][256] = 
      {"white", "black", "red", "orange", "yellow",
       "green", "darkgreen", "blue", "navyblue", "darkviolet"};
   ...
   while(!quit) {
	 ...
	 switch(event.type) {
      ...
      if (splashScreen) {
         ...
         handleAnimation(bVector, *paddle, *ball, *score, xinfo, release, quit);
         repaint(gameScreenDVector, xinfo);
      } // if
